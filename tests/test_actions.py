...
from src.physics.action_losses import (higgs_potential_loss,
...
def _sm_lattice():
    return SM.create_lattice(SHAPE)


def test_wilson_action_zero_on_cold_vacuum():
    edge_index, _, _, _ = _sm_lattice()
    plaq = SM.find_rectangular_loops(SHAPE, edge_index)
    calc = SM.WilsonAction(plaq, group_dim=3, beta=2.5)
    u = torch.eye(3, dtype=torch.complex64).expand(edge_index.size(1), 3, 3)
    assert abs(calc(u).item()) < 1e-4


def test_wilson_action_gauge_invariant():
    torch.manual_seed(0)
    edge_index, _, is_fwd, partner = _sm_lattice()
    plaq = SM.find_rectangular_loops(SHAPE, edge_index)
    calc = SM.WilsonAction(plaq, group_dim=2, beta=1.7)
    raw = torch.randn(edge_index.size(1), 2, 2, dtype=torch.complex64)
    u = SM.get_gate(raw, partner, is_fwd, is_su=True)
    g = random_su(2, V, seed=7)
    u_t = gauge_transform_links(u, g, edge_index)
    assert torch.allclose(calc(u), calc(u_t), rtol=1e-4)


def test_higgs_action_gauge_invariant():
    torch.manual_seed(0)
    edge_index, _, is_fwd, partner = _sm_lattice()
    phi = torch.randn(V, 2, dtype=torch.complex64)
    u2 = SM.get_gate(torch.randn(edge_index.size(1), 2, 2, dtype=torch.complex64),
                     partner, is_fwd, is_su=True)
    u1 = SM.get_gate(torch.randn(edge_index.size(1), 1, 1, dtype=torch.complex64),
                     partner, is_fwd, is_su=False)
    calc = SM.HiggsAction(v=1.0, lam=0.3)
...
def test_src_plaquette_loss_zero_on_cold_vacuum_and_invariant():
...
def test_higgs_potential_minimum_at_target_vev():
    v = 1.3
