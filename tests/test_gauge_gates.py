...
from src.physics.gauge_groups import (SMGateFactory, exp_sm_algebra_to_group,
                                      get_sm_generators)


def test_get_gate_unitary_su_det_and_reversal():
    torch.manual_seed(0)
    edge_index, _, is_fwd, partner = SM.create_lattice((4, 4))
...
def test_get_gate_u1_is_pure_phase():
    torch.manual_seed(0)
    edge_index, _, is_fwd, partner = SM.create_lattice((4, 4))
    raw = torch.randn(edge_index.size(1), 1, 1, dtype=torch.complex64)
    u = SM.get_gate(raw, partner, is_fwd, is_su=False)
    assert torch.allclose(u.abs(), torch.ones_like(u.abs()), atol=1e-5)


def test_exp_algebra_produces_group_elements():
    torch.manual_seed(0)
    gens = get_sm_generators('cpu')
    a = {'su3': torch.randn(10, 8), 'su2': torch.randn(10, 3),
         'u1': torch.randn(10, 1)}
...
def test_sm_gate_factory_gates_are_unitary():
    torch.manual_seed(0)
    gens = get_sm_generators('cpu')
    a = {'su3': torch.randn(6, 8), 'su2': torch.randn(6, 3),
         'u1': torch.randn(6, 1)}
    u3, u2, u1 = exp_sm_algebra_to_group(a, gens)
